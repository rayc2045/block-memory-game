...
class Blocks {
  constructor(blockAssign, soundAssign) {
    // In map(), use () to include {} and return an object. If not, browser will see {} as a function
  ...
  getAudioObject(pitch) {
    return new Audio(
      `https://raw.githubusercontent.com/rayc2045/block-memory-game/main/audio/piano_sound_${pitch}.wav`
    );
  }
...
class MemoryGame {
  ...
  events() {
    ...
    this.restartElement.addEventListener('click', () => {
      this.wrapperElement.classList.remove('blur');
  ...
  startListening() {
    ...
    this.timer = setInterval(() => {
      ...
      if (!notesArray.length) {
        ...
        setTimeout(() => this.startInputting(), this.playInterval);
      }
      // console.log(note);
  ...
  showInputProgressCircles(tempString) {
    ...
    // Update circle div elements in any level
  ...
  gameContinue() {
    ...
    setTimeout(() => this.blocks.turnAllOff(), this.playInterval + 400);
    setTimeout(() => this.startNewLevel(), this.playInterval + 600);
  }
  ...
  gameOver() {
    ...
    setTimeout(() => {
      ...
      this.wrapperElement.classList.add('blur');
